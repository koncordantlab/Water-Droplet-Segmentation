...
| `NASA_CORS_ORIGINS` | `http://localhost:3000` | Comma-separated list of origins allowed to call `/api/*` with credentials. **A literal `*` makes `create_app()` refuse to start (`RuntimeError`)**: flask-cors reflects the request `Origin` header (because `send_wildcard` defaults to False); combined with `supports_credentials=True`, which sends `Access-Control-Allow-Credentials: true`, this would let any site make credentialed requests — i.e. it would defeat the purpose of the credentials check. |
...
- `GET /api/events/<task_id>` is a `text/event-stream` endpoint that drains that queue and emits `data: {...}\n\n` SSE frames. The React `App.js` opens an `EventSource` on this URL and updates progress/results from the stream. Two final frames matter: the pipeline's `completed` progress event (carries `charts`, `rows`, `overlap_totals`, `excel_path`, `execution_time`, `size_distribution` — this is the event the React app folds, after which it closes the EventSource) and the worker's `finished` frame (`data` payload with `overlaps` (note: not `overlap_totals`), `download_url`, `charts`, `rows`, `excel_path`, `execution_time`, `size_distribution`, and the same structure as the `completed` event — the API-contract payload pinned by `tests/unit/test_api.py`). Known pre-existing gap: because React closes on `completed`, the opaque `download_url` never reaches the browser and the download link falls back to the raw `excel_path` (non-functional) — behavior fix deferred to a follow-up PR. If the worker goes quiet, the stream emits `: keep-alive\n\n` comments (ignored by `EventSource`) every `SSE_IDLE_TIMEOUT` (300s), capped at `SSE_MAX_KEEPALIVES` (12, i.e. ~1h total) — beyond that the worker is declared hung, the stream errors out, and the task entry is reclaimed instead of leaking forever.
