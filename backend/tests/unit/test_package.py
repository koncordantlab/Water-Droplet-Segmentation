"""The droplet_backend package imports with no side effects (no YOLO, no Flask app)."""
import os
import sys
...
def test_config_constants_and_weights_default(monkeypatch):
    ...
    monkeypatch.delenv("DROPLET_WEIGHTS_PATH", raising=False)
    assert config.weights_path() == config.DEFAULT_WEIGHTS_PATH
    monkeypatch.setenv("DROPLET_WEIGHTS_PATH", "/somewhere/w.pt")
...
def test_package_import_is_side_effect_free():
    ...
    code = (
        "import sys; import droplet_backend.config; "
