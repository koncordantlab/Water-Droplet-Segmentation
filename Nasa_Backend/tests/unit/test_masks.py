...
def test_resize_bin_masks_matches_cv2_exactly(device):
    ...
    for (sh, sw, dh, dw) in _SHAPE_CASES:
        for n in (1, 3, 7):
            ...
            got = masks._resize_bin_masks_nn(
...
def test_threshold_matches_numpy_at_boundary(device):
...
def test_index_map_is_consistent_and_cached():
    ...
    a = masks._nn_resize_index_map(160, 1920)
    b = masks._nn_resize_index_map(160, 1920)
...
def test_gpu_output_matches_cpu_and_cv2():
    ...
    for (sh, sw, dh, dw) in [(160, 160, 1080, 1920), (104, 152, 513, 761), (200, 200, 100, 100)]:
        ...
        cpu_out = masks._resize_bin_masks_nn(t, dh, dw, thresh=0.3).cpu().numpy()
        gpu_out = masks._resize_bin_masks_nn(t.cuda(), dh, dw, thresh=0.3).cpu().numpy()
        ...
        E_cpu = masks._overlap_exists_matrix(flat).cpu().numpy()
        E_gpu = masks._overlap_exists_matrix(flat.cuda()).cpu().numpy()
        ...
        binm = masks._threshold_masks(t, 0.3)
        ar_cpu = [int(v) for v in masks._mask_areas_from_source(binm, dh, dw)]
        ar_gpu = [int(v) for v in masks._mask_areas_from_source(binm.cuda(), dh, dw)]
...
def test_overlap_exists_matrix_matches_np_any(device):
    ...
    for trial in range(25):
        ...
        mask_array = (rng.random((n, p)) > 0.7).astype(np.uint8)  # sparse: some pairs miss
        if trial % 7 == 0 and n:
            mask_array[0] = 0  # force an all-zero mask
        E = masks._overlap_exists_matrix(torch.from_numpy(mask_array).to(device)).cpu().numpy()
        for k in range(n):
            for j in range(n):
                expect = bool(np.any(mask_array[k] & mask_array[j]))
...
def test_overlap_source_res_equals_full_res_when_upscaling(device):
    ...
    for (sh, sw, dh, dw) in [(160, 160, 1080, 1920), (96, 168, 480, 640), (50, 80, 50, 80)]:
        ...
        E_src = masks._overlap_exists_matrix(
            torch.from_numpy(binm).to(device).reshape(n, -1)
        ).cpu().numpy()
        E_full = masks._overlap_exists_matrix(
        ...
        assert masks._classify_overlaps(E_src, names) == ref, (
        ...
        for k in range(n):
            for j in range(k + 1, n):
                ...
                if cls[k] == cls[j] == "water":
                    ...
                    mixed += 1
        assert masks._classify_overlaps(E, cls) == (ww, ii, mixed), (
...
def test_mask_areas_from_source_matches_cv2_sum(device):
    ...
    for (sh, sw, dh, dw) in _SHAPE_CASES:
        for n in (1, 4):
            ...
            got = [int(v) for v in masks._mask_areas_from_source(binm, dh, dw)]
            assert got == expected, (
